...
class ModelExtensionExchange1c extends Model {
    ...
    private $ERROR			= 0;
    private $XML_VER		= "";
    private $FEATURE_R_GUID = '';
    private $FEATURE_R_VALUE = '';
    ...
    private function setOptionValue($value, $option_id, $image = '', $sort_order = 0) {
        ...
        $query_other_lang = $this->query("SELECT * FROM " . DB_PREFIX . "language WHERE language_id != " . $this->LANG_ID)->rows;
        ...
        if ($option_value_id) {
            ...
            foreach ($query_other_lang as $ol) {
                $this->query("INSERT INTO `" . DB_PREFIX . "option_value_description` SET `option_id` = " . (int)$option_id . ", `option_value_id` = " . (int)$option_value_id . ", `language_id` = " . $ol['language_id'] . ", `name` = '" . $this->db->escape($value) . "'");
            }
...
//		}
    ...
    private function parseImages($xml) {
        ...
        foreach ($xml as $image) {
            ...
            if (!file_exists(DIR_IMAGE . 'catalog/import/' . $image) && $this->config->get('exchange1c_product_images_check')) {
    ...
    private function updateProductAttributes($product_id, $attributes) {
        ...
        foreach ($query_other_lang as $ol) {
            $query_ol = $this->query("
				SELECT `attribute_id`,`text` FROM `" . DB_PREFIX . "product_attribute` WHERE `product_id` = " . (int)$product_id . " AND `language_id` != " . $ol['language_id']);

            foreach ($query_ol->rows as $attribute) {
                $ol_attributes[$ol['language_id']][$attribute['attribute_id']] = $attribute['text'];
            }
        }
        ...
        foreach ($attributes as $attribute) {
            ...
            foreach ($query_other_lang as $ol) {
                $alternative_name = $this->getAlternativeAttribute($product_id, $attribute['attribute_id'], $attribute['value'], $ol['language_id']);

                if (isset($ol_attributes[$ol['language_id']][$attribute['attribute_id']]) && !is_null($alternative_name)) {

                    if ($ol_attributes[$ol['language_id']][$attribute['attribute_id']] != $alternative_name) {
                        $this->query("UPDATE `" . DB_PREFIX . "product_attribute` SET `text` = '" . $this->db->escape($alternative_name) . "' WHERE `product_id` = " . (int)$product_id . " AND `attribute_id` = " . (int)$attribute['attribute_id'] . " AND `language_id` = " . $ol['language_id']);
                        $this->log("Атрибут товара обновлен'" . $this->db->escape($attribute['name']) . "' = '" . $this->db->escape($attribute['value']) . "' записано в товар id: " . (int)$product_id, 2);
                    } else {
                        $this->query("DELETE FROM `" . DB_PREFIX . "product_attribute` WHERE `product_id` = " . (int)$product_id . " AND `attribute_id` = " . (int)$attribute['attribute_id'] . " AND `language_id` = " . $ol['language_id']);
                        //var_dump($product_id, $attribute['attribute_id'], $attribute['value'], $ol['language_id']);
                        $this->query("INSERT INTO `" . DB_PREFIX . "product_attribute` SET `product_id` = " . (int)$product_id . ", `attribute_id` = " . (int)$attribute['attribute_id'] . ", `language_id` = " . $ol['language_id'] . ", `text` = '" .  $this->db->escape($alternative_name) . "'");
                        //var_dump($product_id);
                        //$this->log("Атрибут товара добавлен '" . $this->db->escape($attribute['name']) . "' = '" . $this->db->escape($attribute['value']) . "' записано в товар id: " . (int)$product_id, 2);
                    }

                    unset($ol_attributes[$ol['language_id']][$attribute['attribute_id']]);
                } else {
                    $alternative_name = is_null($alternative_name) ? $attribute['value'] : $alternative_name;
                    // Добавим в товар
                    $this->query("DELETE FROM `" . DB_PREFIX . "product_attribute` WHERE `product_id` = " . (int)$product_id . " AND `attribute_id` = " . (int)$attribute['attribute_id'] . " AND `language_id` = " . $ol['language_id']);
                    $this->query("INSERT INTO `" . DB_PREFIX . "product_attribute` SET `product_id` = " . (int)$product_id . ", `attribute_id` = " . (int)$attribute['attribute_id'] . ", `language_id` = " . $ol['language_id'] . ", `text` = '" .  $this->db->escape($alternative_name) . "'");
                    //$this->log("Атрибут товара добавлен '" . $this->db->escape($attribute['name']) . "' = '" . $this->db->escape($attribute['value']) . "' записано в товар id: " . (int)$product_id, 2);
    ...
    private function parseProducts($xml) {
        ...
        foreach ($xml->Товар as $num => $product) {
            ...
            $this->FEATURE_R_GUID = $data['feature_guid'];
            ...
            if ($product->ХарактеристикиТовара && $this->config->get('exchange1c_product_feature_import') == 1) {
                ...
                $options = $this->setProductBundleOptions($product_id, $data_options);
    ...
    private function parseQuantity($xml) {
        ...
        $this->log("Найдено Количество " . $quantity);
    ...
    private function parseDimensions($xml) {
        $dimensions = [];

        if(isset($xml->Длина)) {
            $dimensions['length'] = (float)str_ireplace(',', '.', $xml->Длина);
        }
        if(isset($xml->Ширина)) {
            $dimensions['width'] = (float)str_ireplace(',', '.', $xml->Ширина);
        }
        if(isset($xml->Высота)) {
            $dimensions['height'] = (float)str_ireplace(',', '.', $xml->Высота);
        }

        if(isset($xml->Вес)) {
            $dimensions['weight'] = (float)str_ireplace(',', '.', $xml->Вес);
        }
...
//				}
                ...
                if($price > 0) {
                    $this->query("UPDATE `" . DB_PREFIX . "product` SET `price` = '" . $price . "' WHERE `product_id` = '" . $product_id . "'");
                }
    ...
    private function setProductFeature($product_id, $data) {
        ...
        $query = $this->query("SELECT * FROM `" . DB_PREFIX . "product_feature` WHERE `guid` = '" . $this->db->escape($data['feature_guid']) . "' AND product_id = '" . (int)$product_id . "'");
        if ($query->num_rows) {
            $update_fields = $this->compareArrays($query,$data);
            if ($update_fields) {
    ...
    private function parseProductOptions($xml) {
        ...
        if ($this->config->get('exchange1c_product_options_mode') == 'related') {
            $feature_name = '';
            $feature_value = '';
            ...
            foreach ($xml->ХарактеристикаТовара as $product_option) {
                ...
                if ($this->FEATURE_R_VALUE) {
                    $new_value = stripos($this->FEATURE_R_VALUE, $value);
                    if ($new_value !== false) {
                        $value = $this->FEATURE_R_VALUE;
                    }
                }
    ...
    private function setProductBundleOptions($product_id, $options, $quantity = 0, $image = '', $product_feature_id = 0, $dimensions = [], $status) {
        if (empty($options)) {
            $this->errorLog(2310);
            return false;
        }
        ...
        foreach ($options as $option_name => $option_value_data) {
            ...
            $this->query("
            INSERT IGNORE INTO " . DB_PREFIX . "may_advanced_option_product_config
            SET product_id = '" . (int)$product_id . "',
                option_name = '" . $this->db->escape($option_value_data['bundle_name']) . "',
                subtract = '1',
                swatch_image = '0',
            ...
            if(count($options) > 1 && $combination_id == '0') {
                $data['quantity'] = 0;
            } else {
            ...
            if ($product_id) {
                $query = $this->query("SELECT * FROM `" . DB_PREFIX . "product_option` WHERE `product_id` = " . $product_id . " AND value = 'may_advanced_option' AND `option_id` = " . $option_id . " ORDER BY product_option_id DESC");
                if ($query->num_rows) {
                    $product_option_id = $query->row['product_option_id'];
                    $combination = explode('-', $combination_id);

                    $id = array_pop($combination);

                    if($id != 0) {
                        $query2 = $this->query("SELECT * FROM `" . DB_PREFIX . "may_advanced_option_product_value` WHERE `product_id` = " . $product_id . " AND option_id = '" . (int)$option_id . "' AND `combination_id` = '" . $combination_id . "'");
                        if(!$query2->num_rows) {
                            $this->query("INSERT INTO `" . DB_PREFIX . "product_option` SET `product_id` = " . $product_id . ", value = 'may_advanced_option', `option_id` = " . $option_id . ", `required` = 1");
                            $product_option_id = $this->db->getLastId();
                        } else {
                            $product_option_id = $query2->row['product_option_id'];
                        }
                    }
                } else {
                    $this->query("INSERT INTO `" . DB_PREFIX . "product_option` SET `product_id` = " . $product_id . ", value = 'may_advanced_option', `option_id` = " . $option_id . ", `required` = 1");
                ...
                $query = $this->query("SELECT * FROM `" . DB_PREFIX . "product_option_value` WHERE `product_id` = " . $product_id . " AND  `product_option_id` = " . $product_option_id . " AND `option_value_id` = " . $option_value_id);

                if (!$query->num_rows) {
                $this->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$option_id . "', option_value_id = '" . (int)$option_value_id . "', quantity = '" . (int)$quantity . "', subtract = '1'");
                $product_option_value_id = $this->db->getLastId();
               	} else {
...
// 			}
            if ($offer->Остатки || $offer->Количество || $offer->Склад) {
                $data['quantity'] = $this->parseQuantity($offer);
                if ($this->ERROR) return $num_offer;
            }
            ...
            if ($data['feature_guid']) {
                ...
                $this->FEATURE_R_VALUE = $data['feature_name'];
                ...
                if ($offer->ХарактеристикиТовара) {
                    ...
                    if ($offer->Размеры) {
                        $dimensions = $this->parseDimensions($offer->Размеры);
                    }
                    ...
                    if($offer->Количество) {
                        ...
                        if((int)$offer->Количество == 0) {
                            $status = 0;
                        }
                    }

                    // Сопоставим option_id и option_value_id значеням
                    $this->setProductBundleOptions($product_id, $data_options, $data['quantity'], $feature_image, $product_feature_id, $dimensions, $status);
                    if ($this->ERROR) return $num_offer;
                ...
                if ($this->config->get('exchange1c_product_price_no_import') != 1) {
                    ...
                    foreach ($data_price as $feature_price) {
                        ...
                        foreach ($feature_price as $feature_value) {
                            ...
                            $product_option_info = $this->query("SELECT * FROM `" . DB_PREFIX . "product_option_value` WHERE `product_option_value_id` = " . $feature_value['product_option_value_id']);

                            if ($product_option_info->num_rows) {
                                $product_option_id = $product_option_info->row['product_option_id'];
                                $option_value_id = $product_option_info->row['option_value_id'];
                                $this->log("ВНИМАНИЕ! Опций не найдено для связи!? product_option_value_id: " . $feature_value['product_option_value_id']);
                                $this->query("UPDATE `" . DB_PREFIX . "may_advanced_option_product_value` SET `price` = '" . $feature_value['price'] . "', `price_prefix` = 'as' WHERE `product_option_id` = '" . $product_option_id . "' AND product_id = ' " . $product_id . "' AND option_value_id = '" . $option_value_id . "'");
                            }
            ...
            if ($data['feature_guid']) {
                // Общий остаток 18 июня 2022

                $query = $this->query("SELECT SUM(quantity) as total, MIN(price) as price FROM " . DB_PREFIX . "may_advanced_option_product_value WHERE product_id = '" . (int)$product_id . "' AND hide = '0' AND combination_id != '0'");
                ...
                $this->query("UPDATE " . DB_PREFIX . "product SET quantity = '" . (int)$query->row['total'] . "', price = '" . (float)$query->row['price'] . "'" . $set_status . " WHERE product_id = '" . (int)$product_id . "'");

            }
            // Конец общего остатка
        ...
        $this->query("UPDATE " . DB_PREFIX . "product SET status = '1' WHERE quantity > 0 AND status = 0");

        $combination = $this->query("SELECT product_id, combination_id, SUM(quantity) AS quantity  FROM " . DB_PREFIX . "may_advanced_option_product_value WHERE quantity > 0 AND hide = '0' AND combination_id != '0' GROUP BY product_id, combination_id");

        if($combination->num_rows) {

            foreach ($combination->rows as $combination) {
                $product_id = $combination['product_id'];
                $combination_row = $combination['combination_id'];
                $combination_str = array_reverse(explode('-', $combination['combination_id']));

                foreach ($combination_str as $option_value_id) {
                    if((int)$option_value_id != 0) {
                        $combination_row = str_ireplace('-'.$option_value_id, '', $combination_row);

                        if($combination_row == '0') {
                            $this->query("UPDATE " . DB_PREFIX . "may_advanced_option_product_value SET hide = '0', price = '0.0000', quantity = '" . (int)$combination['quantity'] . "' WHERE product_id = '" . (int)$product_id . "' AND option_value_id = '" . (int)$option_value_id . "' AND combination_id = '" . $combination_row . "'");

                            $this->query("UPDATE " . DB_PREFIX . "product_option_value SET price = '0.0000', quantity = '" . (int)$combination['quantity'] . "' WHERE product_id = '" . (int)$product_id . "' AND option_value_id = '" . (int)$option_value_id . "'");
                        } else {
                            $this->query("UPDATE " . DB_PREFIX . "may_advanced_option_product_value SET hide = '0', price = '0.0000' WHERE product_id = '" . (int)$product_id . "' AND option_value_id = '" . (int)$option_value_id . "' AND combination_id = '" . $combination_row . "'");

                            $this->query("UPDATE " . DB_PREFIX . "product_option_value SET price = '0.0000' WHERE product_id = '" . (int)$product_id . "' AND option_value_id = '" . (int)$option_value_id . "'");
        ...
        $this->query("UPDATE " . DB_PREFIX . "may_advanced_option_product_value SET hide = '0'");
        ...
        $this->load->controller('extension/total/hyper_discount/cron');
...
//			$this->log("Обновлено в документе: Итого",2);
    ...
    private function getAlternativeAttribute($product_id, $attribute_id, $value, $language_id) {
        $attribute_value_query = $this->db->query("
				SELECT attribute_value_id 
				FROM `" . DB_PREFIX . "attribute_value_description` 
			WHERE `attribute_id` = " . (int)$attribute_id . " 
			  AND `name` = '" . $this->db->escape($value) . "'
			  AND `language_id` = '" . $this->LANG_ID . "'
			");

        if ($attribute_value_query->num_rows) {
            $attribute_value_id = $attribute_value_query->row['attribute_value_id'];

            $query_other_name = $this->db->query("
					SELECT name 
					FROM `" . DB_PREFIX . "attribute_value_description` 
            ...
            if ($query_other_name->num_rows && !empty($query_other_name->row['name'])) {
                return $query_other_name->row['name'];
            }
        }
