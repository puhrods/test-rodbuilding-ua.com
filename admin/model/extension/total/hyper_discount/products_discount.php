...
class ModelExtensionTotalHyperDiscountProductsDiscount extends Model {
    public function getProductsDiscountsList() {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "hd_product` ORDER BY id");
        return $query->rows;
    }
    ...
    public function cloneProductsEditor($discount_id) {
        ...
        $query = $this->db->query("SELECT `editor_id` FROM `" . DB_PREFIX . "hd_product_discount_editor` WHERE `id`='$discount_id' ORDER BY editor_id DESC LIMIT 0, 1");
        $new_editor_id=(int)$query->row['editor_id']+1;
        $this->db->query("UPDATE `" . DB_PREFIX . "hd_product_discount_editor` SET `editor_id` = '$new_editor_id',`status`='0' WHERE `id`='$id'");
    }

    public function editProductsDiscount($products_discount) {
        $save_data = array();
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "hd_product_discount_editor WHERE  `discount_id` = '" . (int)$products_discount['discount_id'] . "'");
        ...
        $this->db->query("DELETE FROM `" . DB_PREFIX . "hd_product_discount_editor` WHERE discount_id = '" . (int)$products_discount['discount_id'] . "'");
        ...
        if (isset($products_discount['discount_editor'])) {
            foreach ($products_discount['discount_editor'] as $editor_key => $editor_val) {
                ...
                $discount_count_products = (int)$editor_val['discount_count_products'];
                $discount_priority = $editor_val['discount_priority'];
                if($discount_count_products<2)
                        $discount_count_products=2;
                ...
                $status = (isset($editor_val['status'])) ? (int)$editor_val['status'] : 0;
                $save_time=time();
    ...
    public function getProductsDiscountEditorsById($products_discount_id) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "hd_product_discount_editor` WHERE `id` = '" . (int)$products_discount_id . "'");
        return $query->row;
    }
    
    public function getProductsDiscountEditors($products_discount_id) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "hd_product_discount_editor` WHERE `discount_id` = '" . (int)$products_discount_id . "' ORDER BY id");
        return $query->rows;
    }

    public function deleteProductsDiscount($products_discount_id) {
        $query = $this->db->query("SELECT `id` FROM " . DB_PREFIX . "hd_product_discount_editor WHERE `discount_id`='" . (int)$products_discount_id . "'");

        if($query->num_rows) {
            foreach($query->rows as $key => $value) {
                if ($value['id'] > 0) {
                    $this->db->query("DELETE FROM " . DB_PREFIX . "product_discount WHERE hdpid = '" . (int)$value['id'] . "'");
                }
            }
        }
