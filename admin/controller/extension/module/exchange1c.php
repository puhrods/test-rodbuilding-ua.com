...
//					}
    ...
    public function manualImportAllFiles($file = 'import.xml') {

        $this->load->language('extension/module/exchange1c');
        $cache = DIR_IMAGE . 'catalog/import/';
        $json = array();
        $error = "";

        // Разрешен ли IP
        if ($this->checkAccess()) {
            $import_file = $cache . $file;
            if (!is_file($import_file)) {
                $this->log("Файл импорта по Cron не существует");
                if($file == 'import.xml') {
                    $this->manualImportAllFiles('offers.xml');
                }

                $error = "Cron import filename dont exist";
            }
            else {
                $error = $this->modeImport($import_file);
                
                if(!$error && $file == 'import.xml'){
                    $this->manualImportAllFiles('offers.xml');
                }
            }
	 ...
	 * update 2018-06-27
	 */
	public function cronImport($file = 'import.xml') {
