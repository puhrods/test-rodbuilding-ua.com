...
class ModelOCTemplatesFaqOCTProductFaq extends Model {
	public function addFaq($product_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "oct_faq SET author = '" . $this->db->escape($data['name']) . "', customer_id = '" . (int)$this->customer->getId() . "', product_id = '" . (int)$product_id . "', text = '" . $this->db->escape($data['text']) . "', date_added = NOW()");
